"""GLM-5 Function Calling Tool Registry

Converts LangChain tools to GLM function calling format and provides
...
class GLMTool:
    """Represents a tool in GLM function calling format."""
    ...
    def to_glm_format(self) -> Dict[str, Any]:
        """Convert to GLM function calling format."""
        return {
            "type": "function",
            "function": {
                "name": self.name,
...
def _extract_function_schema(func: Callable) -> Dict[str, Any]:
    ...
    for name, param in sig.parameters.items():
        ...
        if param.default == inspect.Parameter.empty and origin not in (Optional, Union):
...
class ToolRegistry:
    """Registry for GLM function calling tools.
    ...
    def register_from_langchain(self, langchain_tool: Callable) -> None:
        ...
        self._tools[name] = GLMTool(name, description, parameters, handler)
    ...
    def get_tool(self, name: str) -> Optional[GLMTool]:
        """Get a tool by name."""
        return self._tools.get(name)

    def get_all_tools(self) -> List[GLMTool]:
        """Get all registered tools."""
        return list(self._tools.values())

    def to_glm_tools_list(self) -> List[Dict[str, Any]]:
        """Export all tools in GLM function calling format."""
        return [tool.to_glm_format() for tool in self._tools.values()]
